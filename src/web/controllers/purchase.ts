...
export default class PurchaseController {
  ...
  async create(
    ...
    const offers = await OfferModel
      .find({ _id: { $in: offerIds.map(mongoose.Types.ObjectId) } })
      .populate('shop')
      .populate('product')

    if (offers.length !== offerIds.length) throw createHttpError(404)

    const officeIds = data.items.filter(value => !!value.office).map(el => el.office)
    const offices = await OfficeModel
      .find({ _id: { $in: officeIds.map(mongoose.Types.ObjectId) } })
      .populate('shop')
      .populate('user')
      .populate('city')
